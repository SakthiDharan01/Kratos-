...
export async function POST(request: NextRequest) {
  try {
    ...
    // If SMTP fails, return the email content for manual processing
    return NextResponse.json({
      success: false,
      error: 'SMTP service unavailable',
      emailContent: {
        to: (request.body as any)?.email,
        subject: `🎉 Registration Confirmed - KRATOS 2K25`,
        html: generateEmailHTML(request.body as any)
      }
    }, { status: 500 });
  }
}
...
function generateEmailHTML(data: EmailData): string {
  const { name, teamName, eventName, paymentId, amount, teamMembers, qrUrl } = data;
